...
class GCN(TorchModelV2, torch.nn.Module):
    def __init__(
        ...
        self.states_size = obs_space.spaces["states"].shape[0]
        self.num_node_features = obs_space.spaces["node_features"].shape[1]
        self.num_edge_features = obs_space.spaces["edge_attr"].shape[1]
        ...
        self.stats_nn = torch.nn.Sequential(
            torch.nn.Linear(4, 12),
            torch.nn.ReLU()
        )
        # create the LSTM layer
        ACTION_OUT = 8
        self.lstm = torch.nn.LSTM(self.num_outputs, self.lstm_state_size, batch_first=True)
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        assert new_batch_size * time_size == batch_size, "seq_lens and batch size don't add up, input probably has to be zero padded"
        ...
        if "previous_action" in input_dict["obs"]:
            prev_action_one_hot = torch.nn.functional.one_hot(previous_actions.long(), num_classes=self.num_outputs).float()
            prev_action_one_hot = prev_action_one_hot.squeeze(2)
        else:
            prev_action_one_hot = torch.zeros(self.num_outputs)
        ...
        # get the action
        self.intermediate_x = self.state_postprocessing_fc(torch.cat([action_x, graph_x_max, graph_x_mean, states_x], dim=1))

        policy_x = self.policy_fc1(self.intermediate_x)
        policy_x = self.policy_fc2(policy_x)
        
        print()
        return (policy_x, [h.squeeze(0), c.squeeze(0)])
